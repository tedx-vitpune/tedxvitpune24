import Link from "next/link";
import Image from "next/image";

export default function Cards() {
	return (
		<div className='mt-12'>
			<div className='lg:w-4/5 mx-auto py-12 lg:py-16 lg:flex lg:items-center lg:justify-between px-6 lg:px-0'>
				<div className='w-full lg:w-1/2 px-8 lg:px-12 py-8 lg:py-12 lg:mr-4 bg-black/70 backdrop-blur-xl rounded-cc shadow-md'>
					<h2 className='text-2xl heading text-center font-extrabold tracking-tight text-white md:text-4xl'>
						About Us
					</h2>
					<p className='mt-12 text-xl md:text-2xl text-white text-justify'>
						In the spirit of Ideas Worth Spreading, we, at TEDxVITPune, solely
						believe in the power of ideas to change attitudes, lives, and
						ultimately, the world at large. We&apos;re bringing together people
						from all different walks of life to come, confer and confabulate
						ideas in whole different dimensions.
					</p>
					<div className='mt-8 lg:mt-12 flex justify-end'>
						<Link href='/about'>
							<div className='bg-[#dd1313] h-12 w-12 rounded-full flex justify-center items-center cursor-pointer'>
								<Image
									className='w-6 h-6'
									width={24}
									height={24}
									src='/rc.svg'
									alt='chevron-right'
								/>
							</div>
						</Link>
					</div>
				</div>

				<div className='w-full lg:w-1/2 lg:ml-4'>
					<div className='w-full px-8 lg:px-12 py-12 my-8 bg-black/70 backdrop-blur-xl  rounded-[40px] shadow-md'>
						<div className='flex justify-between items-center'>
							<h2 className='text-2xl heading font-extrabold tracking-tight text-white md:text-4xl'>
								Lookbook
							</h2>
							<Link href='/lookbook'>
								<div className='bg-[#dd1313] h-12 w-12 rounded-full flex justify-center items-center cursor-pointer'>
									<Image
										className='w-6 h-6'
										width={24}
										height={24}
										src='/rc.svg'
										alt='chevron-right'
									/>
								</div>
							</Link>
						</div>
					</div>

					<div className='w-full px-8 lg:px-12 py-12 my-8 bg-black/70 backdrop-blur-xl  rounded-[40px] shadow-md'>
						<div className='flex justify-between items-center'>
							<h2 className='text-2xl heading font-extrabold tracking-tight text-white md:text-4xl'>
								Community Conversations
							</h2>
							<Link href='/community-conversations'>
								<div className='bg-[#dd1313] h-12 w-12 rounded-full flex justify-center items-center cursor-pointer'>
									<Image
										className='w-6 h-6'
										width={24}
										height={24}
										src='/rc.svg'
										alt='chevron-right'
									/>
								</div>
							</Link>
						</div>
					</div>

					<div className='w-full px-8 lg:px-12 py-12 my-8 bg-black/70 backdrop-blur-xl  rounded-[40px] shadow-md'>
						<div className='flex justify-between items-center'>
							<h2 className='text-2xl heading font-extrabold tracking-tight text-white md:text-4xl'>
								Blog
							</h2>
							<Link href='/blog'>
								<div className='bg-[#dd1313] h-12 w-12 rounded-full flex justify-center items-center cursor-pointer'>
									<Image
										className='w-6 h-6'
										width={24}
										height={24}
										src='/rc.svg'
										alt='chevron-right'
									/>
								</div>
							</Link>
						</div>
					</div>
				</div>
			</div>
		</div>
	);
}
